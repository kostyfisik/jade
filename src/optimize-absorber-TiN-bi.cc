...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    sign_ = TiN_filename.substr(0, core_filename.find("."))+"-"+
    ...
    // **************  Main loop   ***********************
    // ***************************************************  
    for (total_r_ = step_r_; total_r_ < max_r_*1.0000001; total_r_+=step_r_) {
    // for (total_r_ = 33.0; total_r_ < 40.0; total_r_+=step_r_) {
      // if (total_r_ > 77 && total_r_ < 84) step_r_ = 0.1;
      // else step_r_ = 1.0;
...
void Print() {
  ...
  const double shell_width = total_r_ - TiN_width;
  printf("first_shell_width:%.19g\nouter_shell_width:%.19g\n",
	 TiN_width, shell_width);
...
// ********************************************************************** //
double EvaluateFitness(std::vector<double> input) {
  if (input.size() != 1) throw std::invalid_argument("Wrong input dimension!/n");
  TiN_share_ = input[0];
  const double TiN_width = (total_r_>max_TiN_width_ ? max_TiN_width_ : total_r_) * TiN_share_;
  const double shell_width = total_r_ - TiN_width;
  if (TiN_width <= 0.0 || shell_width <= 0.0) {
    printf("TiN_share = %g\n",TiN_share_);
    printf("total_r_ = %g\n",total_r_);
    printf("TiN <=0:   %g\n", TiN_width);
    printf("shell <=0:   %g\n",shell_width);
...
std::vector< std::vector<double> > EvaluateSpectraForBestDesign() {
  ...
  if (best_x.size() != 1) throw std::invalid_argument("Wrong input dimension!/n");
  TiN_share_ = best_x[0];
  const double TiN_width = (total_r_>max_TiN_width_ ? max_TiN_width_ : total_r_) * TiN_share_;
  const double shell_width = total_r_ - TiN_width;
  if (TiN_width <= 0.0 || shell_width <= 0.0) {
...
void PrintGnuPlotSpectra(std::vector< std::vector<double> > spectra) {
  ...
  snprintf(plot_name, 300,
           "%s-TotalR%06.2fnm-Qabs%016.13f--inshell%07.2fnm--outshell%07.2fnm-fails%d-spectra", sign_.c_str(),
           total_r_, Qabs_, TiN_width, shell_width, fails_);
...
void PrintGnuPlotChannels(std::vector< std::vector<double> > spectra) {
  ...
  const double shell_width = total_r_  - TiN_width;  
  snprintf(plot_name, 300,
           "o-spectra-%s-channels-TotalR%06.2fnm-Qabs%016.13f--inshell%07.2fnm--outshell%07.2fnm-fails%d", sign_.c_str(),
           total_r_, Qabs_,  TiN_width, shell_width, fails_);
  full_sign_ = std::string(plot_name);
  wrapper.SetPlotName(plot_name);
  wrapper.SetXLabelName("WL");
