...
//Only no loss case!!
double epsilon_m(double omega) {
  // std::cout << "omega:" << omega << "  omega_p:"<<omega_p_ <<std::endl;
  return 1.0 - pow2(omega_p_/omega);
}
// Set dispersion
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (double omega = from_omega_; omega < to_omega_; omega += omega_step) {
      double metal_index = std::sqrt(-epsilon_m(omega));
      multi_layer_mie_.ClearTarget();
      multi_layer_mie_.AddTargetLayer(core_width_, {lossless, metal_index });
