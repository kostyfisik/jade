...
#WL_work = 3.75
condition_index = 1.0;
#os.system("rm *-overview-condition"+str(condition_index)+".dat")            
for dirname, dirnames, filenames in os.walk('.'):
    # print path to all filenames.
    for filename in filenames:
        isGood = True
        if '-index.dat' in filename:
            #print filename
            sign = filename[0:-12]
            r =  sign[7:13]
            width = sign[23:28]
            ...
            for num in [1,2,3,4,8,16,32,64]:
                if sign.find('-n'+str(num)+'-') > -1:
                    layers_num= 'R'+str(r)+'-n'+str(num)+'-overview-condition'+str(condition_index)
